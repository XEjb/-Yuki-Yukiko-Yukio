...
class YukikoHome(DataMixin, ListView):
    ...
    def get_queryset(self):
        return Yukiko.objects.filter(is_published=True).select_related('cat')
...
class ContactFormView(DataMixin, FormView):
    ...
    template_name = 'yukiko/contact.html'
    success_url = reverse_lazy('home')

    def get_context_data(self, *, objects_list=None, **kwargs):
        context = super().get_context_data(**kwargs)
        c_def = self.get_user_context(title='Фидбэк')
        return dict(list(context.items()) + list(c_def.items()))
...
class YukikoCategory(DataMixin, ListView):
    ...
    def get_queryset(self):
        return Yukiko.objects.filter(cat__slug=self.kwargs['cat_slug'], is_published=True).select_related('cat')
